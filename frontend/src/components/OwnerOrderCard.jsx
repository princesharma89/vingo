    ...
    <div className='bg-white rounded-lg shadow p-4 space-y-4'>
      ...
      <div className='flex justify-between items-center mt-auto pt-3 border-t border-gray-100'>
        <span className='text-sm'>
          Status:
          <span className='font-semibold capitalize text-[#ff4d2d]'>{data?.shopOrders?.[0]?.status}</span>
          <span>
          </span>
        </span>
        <select value={data?.shopOrders?.[0]?.status} className='rounded-md border px-3 py-1 text-sm focus:outline-none focus:ring-2 border-[#ff4d2d] text-[#ff4d2d]' >
