...
function Nav() {
  ...
  return (
    <div className='w-full h-[80px] flex items-center justify-between md:justify-center gap-[30px] px-[20px] fixed top-0 left-0 z-[999] bg-[#fff9f6] overflow-visible'>
      ...
      {/* search bar */}
      {userData.role == 'user' && <div className=' md:w-[60%] lg:w-[40%] h-[70px] bg-[70px] bg-white shadow-xl rounded-lg items-center gap-[20px] hidden md:flex'>
        {/* location */}
        <div className='flex items-center w-[30%] overflow-hidden gap-[10px] border-r-[2px] border-gray-400 '>
          <FaLocationDot size={25} className=' text-[#ff4d2d]' />
          <div className='w-[80%] truncate text-gray-600'>{currentCity}</div>
      ...
      <div className='flex items-center gap-4'>
        ...
        {userData.role == "user" && (showSearch ? <RxCross2 size={25} className='text-[#ff4d2d] md:hidden cursor-pointer' onClick={() => setShowSearch(false)} /> : <FaSearch size={25} className='text-[#ff4d2d] md:hidden cursor-pointer' onClick={() =>
          setShowSearch(true)
        } />)}
        {userData.role == "owner" ? <>
          {myShopData && <>
